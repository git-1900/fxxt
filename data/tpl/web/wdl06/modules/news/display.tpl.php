<?php defined('IN_IA') or exit('Access Denied');?><div id="modal-module-menus" class="modal hide fade" tabindex="-1" role="dialog" aria-hidden="true" style=" width:600px;">
			...
			<tr>
				...
				<td>
					<div class="input-append" style="display:block;">
						<input type="text" class="span3" name="keyword" value="" id="keyword" /><button class="btn" onclick="buildNewsForm(); return false;">搜索</button>
					</div>
				</td>
			</tr>
		</table>
...
<div id="news-list">
	<?php  if(!empty($reply)) { ?>
		...
		<div id="news-wrap-item-<?php  echo $key;?>" class="alert alert-block alert-new wrap-item">
			...
			<div class="reply-news-edit-button"><a href="javascript:;" onclick="newsHandler.buildForm('<?php  echo $key;?>')" class="btn"><i class="icon-plus"></i> 添加多条内容</a></div>
		</div>
		<?php  } } ?>
	<?php  } ?>
</div>
...
<!--
	...
	function buildNewsForm() {
		...
		try {
			$.ajax({
			  url: "<?php  echo create_url('site/module', array('name' => 'news', 'do' => 'getresource'))?>",
			  type: "GET",
