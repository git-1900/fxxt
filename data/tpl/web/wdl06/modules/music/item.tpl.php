...
<div id="show" class="span6 alert alert-info <?php  if(empty($item)) { ?>hide<?php  } ?>">
	<span class="pull-right">
		<?php  if(!empty($item)) { ?><a href="<?php  echo create_url('site/module/delete', array('name' => 'music', 'id' => $item['id']))?>" onclick="return doDeleteItem('<?php  echo $itemid;?>', this.href)" style="margin-right:5px;">删除</a><?php  } else { ?><a onclick="doDeleteItem('<?php  echo $itemid;?>');" href="javascript:;" style="margin-right:5px;">删除</a><?php  } ?>
...
<div id="form" class="alert alert-block alert-new <?php  if(!empty($item)) { ?>hide<?php  } ?>">
	...
	<table>
		...
		<tr>
			...
			<td>
				...
				<div class="input-append">
					<input id="item-url" class="span6" type="text" name="music-url<?php  echo $namesuffix;?>" value="<?php  echo $item['url'];?>" placeholder="">
					<input type="button" id="music-attach-btn" fieldname="music-attach<?php  echo $namesuffix;?>" class="btn" value="<i class='icon-upload-alt'></i>" style="font-size:14px;width:55px;margin-left:-1px;">
				</div>
		...
		<tr>
			<th></th>
			<td><button type="button" onclick="musicHandler.doAdd('<?php  echo $itemid;?>')" class="btn btn-primary span2">完成</button></td>
		</tr>
	</table>
</div>