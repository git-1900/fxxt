...
<ul class="nav nav-tabs">
	<li <?php  if($operation == 'post'&&$modules!='category') { ?>class="active"<?php  } ?>><a href="<?php  echo $this->createWebUrl('goods', array('op' => 'post'))?>">添加商品</a></li>
	<li <?php  if($operation == 'display'&&$modules!='category') { ?>class="active"<?php  } ?>><a href="<?php  echo $this->createWebUrl('goods', array('op' => 'display'))?>">管理商品</a></li>
	<li <?php  if($operation == 'post'&&$modules=='category') { ?>class="active"<?php  } ?>><a href="<?php  echo $this->createWebUrl('category', array('op' => 'post'))?>">添加商品分类</a></li>
	<li <?php  if($operation == 'display'&&$modules=='category') { ?>class="active"<?php  } ?>><a href="<?php  echo $this->createWebUrl('category', array('op' => 'display'))?>">管理商品分类</a></li>
		...
		<table class="table table-bordered tb">
			<tbody>
				...
				<tr>
					...
					<td>
						<select name="status">
							<option value="1" <?php  if(!empty($_GPC['status'])) { ?> selected<?php  } ?>>销售中</option>
							<option value="0" <?php  if(empty($_GPC['status'])) { ?> selected<?php  } ?>>已下架</option>
						</select>
					</td>
				</tr>
				<tr>
		...
		<table class="table table-hover">
			...
			<tbody>
				...
				<tr>
					...
					<td><?php  if(!empty($category[$item['pcate']])) { ?><span class="text-error">[<?php  echo $category[$item['pcate']]['name'];?>] </span><?php  } ?><?php  if(!empty($children[$item['pcate']]) && $children[$item['pcate']][$item['ccate']]['1']!="") { ?><span class="text-info">[<?php  echo $children[$item['pcate']][$item['ccate']]['1'];?>] </span><?php  } ?><?php  echo $item['title'];?>
                                        
                                        
                                        </td>
                                        <td>
                                       <label data='<?php  echo $item['istime'];?>' class='label <?php  if($item['istime']==1) { ?>label-info<?php  } ?>' >限时秒杀</label>
                                        <label data='<?php  echo $item['issendfree'];?>' class='label <?php  if($item['issendfree']==1) { ?>label-info<?php  } ?>' onclick="setProperty(this,<?php  echo $item['id'];?>,'sendfree')">包邮</label>
                                       <!-- <label data='<?php  echo $item['ishot'];?>' class='label <?php  if($item['ishot']==1) { ?>label-info<?php  } ?>' onclick="setProperty(this,<?php  echo $item['id'];?>,'hot')">热卖</label>-->
                                        <label data='<?php  echo $item['isrecommand'];?>' class='label <?php  if($item['isrecommand']==1) { ?>label-info<?php  } ?>' onclick="setProperty(this,<?php  echo $item['id'];?>,'recommand')">首页推荐</label>
					...
					<td><?php  if($item['status']) { ?><span data='<?php  echo $item['status'];?>' onclick="setProperty1(this,<?php  echo $item['id'];?>,'status')" class="label label-success" style="cursor:pointer;">销售中</span><?php  } else { ?><span data='<?php  echo $item['status'];?>' onclick="setProperty1(this,<?php  echo $item['id'];?>,'status')" class="label label-error" style="cursor:pointer;">已下架</span><?php  } ?><!--&nbsp;<span class="label label-info"><?php  if($item['type'] == 1) { ?>实体商品<?php  } else { ?>虚拟商品<?php  } ?></span>--></td>
					<td style="text-align:right;">
