...
<table class="tb">
 ...
 <tr>
    ...
    <?php  if($cfg['globalCommissionLevel']>=3) { ?>
     ...
     <?php  } ?>
	 <tr style="display:none">
        <th><label for="">包邮条件</label></th>
        <td>
            <div class="input-append">
			 <span class="add-on">满数量(件)</span>
                <input type="text" name="msl"  class="span3" value="<?php   echo $item['msl'];?>" /> 

				<span class="add-on">满额（元）</span>
                <input type="text" name="mane"  class="span3" value="<?php   echo $item['mane'];?>" />
				 
            </div>
            <p class="help-block">满数量,满额都包邮!</p>
        </td>
    </tr>
	 <tr>
