...
<ul class="nav nav-tabs">
	<li><a href="<?php  echo create_url('site/nav', array('position' => 3, 'name' => $modulename));?>">快捷导航管理</a></li>
	<li class="active"><a href="<?php  echo create_url('site/shortcut');?>">快捷导航风格</a></li>
	<li><a href="<?php  echo create_url('site/nav/post', array('position' => 3));?>"><i class="icon-plus"></i> 添加导航条目</a></li>
</ul>
		...
		<tr>
			...
			<td>
				<select name="template" onchange="$('#preview').attr('src', '<?php  echo create_url('site/shortcut/preview')?>&file='+this.options[this.options.selectedIndex].value);$('#preview').document.reload();" autocomplete="off">
					...
					<option value="<?php  echo $path;?>" <?php  if(!empty($_W['account']['quickmenu']['enablemodule']) && is_array($_W['account']['quickmenu']['enablemodule']) && ($_W['account']['quickmenu']['template'] == $path)) { ?> selected<?php  } ?>><?php  echo $path;?></option>
					<?php  } } ?>
				</select>
		...
		<tr>
			...
			<td>
				...
				<div style="overflow:hidden;">
					...
					<label class="checkbox" style="width:200px; float:left;"><input type="checkbox" name="module[]" value="<?php  echo $item['name'];?>" <?php  if(!empty($_W['account']['quickmenu']['enablemodule']) && is_array($_W['account']['quickmenu']['enablemodule']) && in_array($item['name'], (array)$_W['account']['quickmenu']['enablemodule'])) { ?> checked<?php  } ?> /> <?php  echo $item['title'];?></label>
					<?php  } ?>
					<?php  } } ?>
				</div>
