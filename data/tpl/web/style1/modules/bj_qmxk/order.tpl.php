...
<!--				<th style="width:50px;">类型</th>-->
                    ...
                    <?php  } ?>
					<?php  if(empty($shareid)) { ?>
					<th style="width:120px;">操作<?php  if($status==1) { ?>&nbsp;&nbsp;<button type="buttom" onclick="location.href='<?php  echo $this->createWebUrl('order', array('op' => 'display', 'status' => 1,'dobatch'=>1))?>';" name="sendbatexpress" value="sendbatexpress" class="btn btn-warning btn-lg">批量发货 <?php  } ?></th>
					<?php  } ?>
				</tr>
			</thead>
			<tbody>
				...
				<tr>
					...
					<?php  if(empty($shareid)) { ?>
					<td><?php  if(empty($item['dispatch'])) { ?>自提<?php  } else { ?>快递配送<?php  } ?></td>
