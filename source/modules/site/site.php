...
class SiteModuleSite extends WeModuleSite {
	...
	public function doWebArticle() {
		...
		if ($foo == 'display') {
			...
			message('删除成功！', referer(), 'success');
                    ...
                    $id = intval($_GPC['id']);
                    $member = pdo_fetchall(" select am.*,qm.status qm_status,qm.flag,card.card_name from ".tablename("activity_members") . " am "
                            . "left join ". tablename("bj_qmxk_member")." qm on am.from_user=qm.from_user and am.weid=qm.weid "
                            . "left join ". tablename("bj_qmxk_card_type")." card on card.cardid=qm.vip and card.weid=qm.weid "
                            . "where am.weid=:weid and am.articleid = :articleid and am.status = 1",array(':weid'=>$_W['weid'],':articleid'=>$id));
                    $total = pdo_fetchcolumn('select count(id) from' . tablename('activity_members') . " where weid=:weid and articleid = :articleid and status = 1",array(':weid'=>$_W['weid'],':articleid'=>$id));
                    include $this->template("activity_members");
                }
	}
	...
	public function doMobileDetail() {
		...
		$id = intval($_GPC['id']);
                //浏览量+1
                $click_add = pdo_query("update ims_article set clicktimes=clicktimes+1 where id=".$id);
		$userinfo = json_decode(base64_decode($_GPC['__msess']), true);
                $activity_exit = pdo_fetchcolumn("select * from ".tablename("activity_members")." where weid=:weid and articleid=:articleid and from_user=:from_user and status=1" ,array(':weid'=>$_W['weid'],':articleid'=>$id,':from_user'=>$userinfo['openid']));
		$sql = "SELECT a.*,ac.name pcate_name FROM " . tablename('article') . " a left join ". tablename("article_category")." ac on a.pcate=ac.id WHERE a.`id`=:id";
        ...
        public function doMobileActivity(){
            ...
            if ($activity_exit<=0) {
                if(checksubmit('submit')){
                    ...
                    pdo_insert('activity_members', $data);
                    $id_1 = pdo_insertid();
                    if ($id_1 >0) {
                        message('报名成功！', $this->createMobileUrl('detail', array('id' => $id)), 'success');
                    } else {
                        message('报名失败！', referer(), 'error');
                    }
                }
            } else {
                message('已报名，无须重复操作！', $this->createMobileUrl('detail', array('id' => $id)), 'error');
            }
