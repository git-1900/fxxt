...
if ($info['subscribe'] == 1) {
    $follow = 1;
    pdo_update("fans",array('follow'=>1),array('from_user'=>$from_user));
} else {
    $follow = 0;
}
...
$rlist = pdo_fetchall('SELECT * FROM ' . tablename('bj_qmxk_goods') . " WHERE weid = '{$_W['weid']}'  and deleted=0 AND status = '1' {$condition} ORDER BY displayorder DESC, sales DESC ");
$cfg = $this->module['config'];
