...
$sort = empty($_GPC['sort']) ? 3 : $_GPC['sort'];
...
$list = pdo_fetchall('SELECT * FROM ' . tablename('bj_qmxk_goods') . " WHERE weid = '{$_W['weid']}'  and deleted=0 AND status = '1' {$condition} ORDER BY {$sortfield}  ");
foreach ($list as &$r) {
    if ($r['istime'] == 1) {
...
unset($r);
$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('bj_qmxk_goods') . " WHERE weid = '{$_W['weid']}'  and deleted=0  AND status = '1' {$condition}");
