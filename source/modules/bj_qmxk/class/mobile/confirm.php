...
if (!empty($id)) {
    $item = pdo_fetch('select id,thumb,ccate,title,weight,marketprice,total,type,totalcnf,sales,unit,istime,timeend,issendfree,msl,mane from ' . tablename('bj_qmxk_goods') . ' where id=:id limit 1', array(':id' => $id));
    foreach ($promotion as $pro) {
	   ...
	   if ($item['msl']>0 && $total >= $item['msl']) {
			$issendfree = 1;
		}
		 if ( $item['mane']>0 && $item['marketprice'] >= $item['mane']) {
			$issendfree = 1;
		}
...
if (!$direct) {
    ...
    if (!empty($list)) {
        foreach ($list as &$g) {
            $item = pdo_fetch('select id,thumb,ccate,title,weight,marketprice,total,type,totalcnf,sales,unit,issendfree,msl,mane from ' . tablename('bj_qmxk_goods') . ' where id=:id limit 1', array(':id' => $g['goodsid']));
			...
			if ($item['msl']>0 && $item['total'] >= $item['msl']) {
				$issendfree = 1;
			}
			 if ( $item['mane']>0 && $item['totalprice'] >= $item['mane']) {
				$issendfree = 1;
			}
        }
        foreach ($promotion as $pro) {
