...
class bj_qmxkModuleCore extends bj_qmxkModuleCommon
{
    ...
    public function shareClick($mid, $joinway = 0)
    {
        ...
        }else if (!empty($mid) && $users['flag']==0) {
            if (!empty($share['sharemid'])) {
                pdo_update('bj_qmxk_share_history', array('sharemid' => $mid),array('weid' => $_W['weid'], 'from_user' => $fromuser));       
            }else{
                pdo_insert('bj_qmxk_share_history', array('sharemid' => $mid,'weid' => $_W['weid'], 'from_user' => $fromuser));
            }
            pdo_update('bj_qmxk_member', array('shareid' => $mid,'createtime'=>time()),array('weid' => $_W['weid'], 'from_user' => $fromuser));
        }
    }
    public function sendMobilePayMsg($order, $goods, $paytype, $ordergoods)
    {
        ...
        if (!empty($template_id)) {
            include IA_ROOT . BJ_QMXK_BASE . '/messagetemplate/pay_TM00001.php';
    ...
    public function autoRegedit($fromaction)
    {
        ...
        if (empty($profile['id'])) {
            ...
            if ($theone['promotertimes'] == 1) {
                ...
                $data = array('weid' => $_W['weid'], 'from_user' => $from_user, 'realname' => $nickname, 'commission' => 0, 'createtime' => TIMESTAMP, 'shareid' => $seid, 'status' => 1, 'flag' => 0);
            }
...
//            if ($profile['flag']==0) {
//                pdo_update('bj_qmxk_member', array('shareid'=>$shareids['sharemid']), array('id' => $profile['id']));
//            }
        }
        $profile = $this->getProfile();
