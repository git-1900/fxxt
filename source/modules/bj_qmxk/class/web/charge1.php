...
if ($op == 'list') {
    if ($_GPC['submit'] == '搜索') {
        ...
        foreach ($list as $key => $item) {
            $member = pdo_fetch('SELECT * FROM ' . tablename('bj_qmxk_member') . ' WHERE  weid = :weid  AND credit2<>0 AND from_user = :from_user limit 1', array(':weid' => $_W['weid'], ':from_user' => $item['from_user']));
            $credit2 = 0;
        ...
        include $this->template('charge1');
        die;
    }
    if (intval($_GPC['so']) == 1) {
        $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('bj_qmxk_member') . ' WHERE weid = :weid  and credit2<>0   and realname<>\'\'  ', array(':weid' => $_W['weid']));
        $pager = pagination($total, $pindex, $psize);
        $list = pdo_fetchall('SELECT * FROM ' . tablename('bj_qmxk_member') . '  WHERE weid=' . $_W['weid'] . '  and credit2<>0   and realname<>\'\'   ORDER BY id DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize);
		 $total = pdo_fetchcolumn('SELECT sum(credit2) FROM ' . tablename('bj_qmxk_member') . ' WHERE weid = :weid  and credit2<>0   and realname<>\'\'  ', array(':weid' => $_W['weid']));
