...
class bj_qmxkModuleReceiverCore extends WeModuleReceiver
{
    ...
    public function get_weixin_token()
    {
        ...
        if (is_array($account['access_token']) && !empty($account['access_token']['token']) && !empty($account['access_token']['expire']) && $account['access_token']['expire'] > TIMESTAMP) {
            ...
            if (empty($appid) || empty($secret)) {
                message('����д���ںŵ�appid��appsecret, (��Ҫ��ĺ���Ϊ΢�ŷ����)��', create_url('account/post', array('id' => $account['weid'])), 'error');
            }
            $url = "https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid={$appid}&secret={$secret}";
            $content = ihttp_get($url);
            if (empty($content)) {
                message('��ȡ΢�Ź��ں���Ȩʧ��, ���Ժ����ԣ�');
            }
            $token = @json_decode($content['content'], true);
            if (empty($token) || !is_array($token)) {
                message('��ȡ΢�Ź��ں���Ȩʧ��, ���Ժ����ԣ� ����ƽ̨����ԭʼ����Ϊ: <br />' . $token);
            }
...
//            else if(!empty($qmjf_qr['id']) && $users['flag']==0){
//                pdo_update('bj_qmxk_share_history', array('sharemid' => $profile['id']),array('weid' => $weid, 'from_user' => $from_user));
//            }
        }
        return false;
    }
