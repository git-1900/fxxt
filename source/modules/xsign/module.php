...
class xsignModule extends WeModule {
	...
	public function fieldsFormDisplay($rid = 0) {
		...
		if (!empty($rid)) {
			...
			$reply = pdo_fetch("SELECT * FROM ".tablename('xsign_reply')." WHERE rid = :rid ORDER BY `id` DESC", array(':rid' => $rid));				
 		} 
	...
	public function fieldsFormSubmit($rid) {
		...
		if (empty($id)) {
			...
			pdo_update('xsign_reply', $insert, array('id' => $id));
	...
	public function ruleDeleted($rid) {
		...
		$replies = pdo_fetchall("SELECT id, picture FROM ".tablename('xsign_reply')." WHERE rid = '$rid'");
		...
		pdo_delete('xsign_reply', "id IN ('".implode("','", $deleteid)."')");
