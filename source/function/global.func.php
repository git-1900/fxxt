...
function checkpermission($type, $target) {
	...
	if ($type == 'wechats') {
		if (is_array($target)) {
			...
			$account = pdo_fetchall("SELECT uid FROM ".tablename('wechats_exp')." WHERE weid = '$target' LIMIT 1");
		}
                $check = false;
...
function getNextMonth($date){
  ...
  if($arr['mon'] == 12){
    (string)$year=$arr['year'] +1;
    (string)$month=$arr['mon'] -11;
    $nextmonth= $year.'-0'.$month;
  }else{
      (string)$year = $arr['year'];
      (string)$month = $arr['mon']+1;
      if ($month>9) {
